...
系统支持两种使用模式：**终端 CLI** 使用 `agentic-qa` 作为启动器；**IDE Chat 集成**（PyCharm / Codex Chat）中由 AI Agent 直接把用户的自然语言指令交给 Runtime 完成 QA 任务。

**纯自然语言入口**：用户指令本体只需要是自然语言，例如“帮我分析 prd/sample-login-requirement 并生成测试用例”。终端场景下 `agentic-qa` 只是启动器，不是用户意图文本的一部分。Runtime 默认先调用 LLM 做语义路由和草稿生成；LLM 不可用时才降级到确定性路由和 Skeleton 草稿。详见 [自然语言 CLI 设计](docs/architecture/natural-language-cli-design.md)。
...
| 声明式资产层 | `prompts/`、`rules/`、`skills/`、`knowledge/` 提供规范约束 |
...
1. 用户输入自然语言指令：“帮我分析 prd/sample-login-requirement 并生成测试用例”。终端场景下通过 `agentic-qa` 启动器传入这句话；IDE/Codex 场景下直接交给 Runtime。
...
- 如果 `prd/<id>/input/requirement.md` 已存在，Runtime 直接使用它，不转换也不覆盖。
- 如果没有 `input/requirement.md`，Runtime 会按固定优先级查找 `requirement.docx`、`requirement.pdf`、`requirement.txt`、`requirement.html`、`requirement.htm`、`requirement.rtf`，以及 `需求.docx`、`需求.pdf`、`需求.txt`、`需求.html`、`需求.htm`。
- 找到受支持源文件后，Runtime 会将其转换为 `prd/<id>/input/requirement.md`，再继续需求分析和测试用例生成。
- 不做目录递归扫描或批量导入，不提交真实业务 Word/PDF 文件；原始需求应只放在目标 PRD 工作区内。

Markdown 轻量清洗策略：

- MarkItDown 是当前默认文档转 Markdown 工具。
- 如果转换后的 `input/requirement.md` 存在乱码、分页符、异常断行、控制字符或连续异常空格，可使用后续轻量清洗脚本生成 `input/requirement.cleaned.md`。
- 清洗只允许去除控制字符、规范空行、清理明显分页符和修复连续异常空格，不得修改业务语义。
- 默认不得覆盖 `input/requirement.md`；需要覆盖时必须由用户显式确认。
- 不做 OCR，不处理图片，不分析原型图；扫描件识别、Pandoc 或 Poppler/pdftotext 接入都属于后续明确授权任务。

## 需求输入与图片策略

- `input/requirement.md`：需求正文，来自 MarkItDown 转换或人工整理。
- `input/api.md`：接口说明，可选。

当前 Runtime 明确不分析图片/原型图内容，不接视觉模型，也不使用 `prototype-notes.md` 作为输入。所有命令只基于 `input/requirement.md` 和 `input/api.md` 的文本生成草稿。

如果 `input/requirement.md` 出现 Markdown 图片引用、`.png`、`.jpg`、`.jpeg`、`media/` 或 `images/` 等图片痕迹，Runtime 会 warning，并在待确认问题中提示人工确认图片中是否存在未写入正文的字段、按钮、状态、弹窗、权限差异或交互规则。Runtime 不会基于图片内容编造字段、按钮、布局或交互。
...
- AI 生成的 QA 报告草稿统一写入 `prd/<id>/report/qa-review.md`。
...
```text
runs/<run_id>/analysis/requirement-analysis.md
runs/<run_id>/cases/test-cases.md
runs/latest.yml
runs/index.jsonl
report/qa-review.md
```
...
```text
帮我分析登录需求 D:\需求\登录.md
帮我分析 prd/sample-login-requirement 并生成测试用例
```
...
建议先设置 LLM 密钥（复制 `.env.example` 为 `.env`，或手动设置环境变量）。未设置时 Runtime 会降级，不直接失败：
...
- "帮我分析登录需求" → 输出 `runs/<run_id>/analysis/requirement-analysis.md`
- "生成测试用例" → 输出 `runs/<run_id>/cases/test-cases.md`
- 最新一轮指针 → `runs/latest.yml`，历史索引 → `runs/index.jsonl`
- "生成接口测试脚本" → 输出 `automation/api/generated/test_login_api.py`
- "执行测试" → 输出 `execution/runs/latest/summary.md`
- "分析失败" → 输出 `defects/failure-analysis.md`
- "生成 QA 报告" → 输出 `report/qa-review.md`
- "确认审核后归档" → 生成归档索引
