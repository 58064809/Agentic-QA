...
当前阶段不实现完整 Agent Runtime、数据库或 Web 平台，而是用 `COMMANDS.md`、`workflows/`、`agents/`、`tasks/`、`prompts/`、`rules/`、`skills/`、`knowledge/` 和 `prd/` 固定 Codex / ChatGPT / IDE Agent 的执行行为。第 2 阶段已开始引入轻量 LangGraph Runtime；Runtime 默认不启用 LLM，仅在用户显式传入 `--use-llm` 且本地环境变量已配置时，使用 OpenAI-compatible adapter 生成草稿。
