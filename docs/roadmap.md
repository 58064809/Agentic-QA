# 路线图

## 当前已实现

- 测试主管与十个声明式专家 Agent。
- `Harness.run/stream/resume/inspect`、薄 CLI 和恢复语义。
- LangGraph 动态并行派发、SQLite checkpoint、真实 interrupt、预算和 partial 降级。
- `workspaces/<id>/` Artifact Store、硬 Review Gate 和原子 promote。
- Agent/Skill/Tool manifest、单 Provider 的 Flash/Pro 集中模型路由、Playwright MCP 工作区配置
  和 run 级 allowlist 快照。
- 随 wheel 发布、由 Skill manifest 显式引用且有路径隔离的通用 QA 知识。
- Workspace/RAG/OpenAPI/API execution/evidence/diff 目标工具。
- API cases v1.1、execution evidence v1、failure triage v1。

## 后续

- 可选 Playwright MCP live smoke CI 环境和更多录制场景。
- LangGraph 持久化后端的 PostgreSQL 运行配置。
- 更多只读测试管理系统连接器；外部缺陷系统写入不在首版范围。
