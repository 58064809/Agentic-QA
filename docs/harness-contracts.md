...
不会把“Prompt 中要求过”当作已经满足。`testcases`、`api_test_draft`、`ui_test_draft`
还必须同时存在无依赖、声明输出的 `requirement_analyst` 与 `risk_strategist` 分析任务，且产物
任务必须直接依赖两类分析；缺少任一前置或依赖时计划不能进入执行。
