...
`run-summary.*` 用于面向人和脚本查看运行摘要；`run-state.json` 与 `graph-state.json` 保存 Runtime 和图状态；`rag.json` 保存召回 trace；`review-events.jsonl` 保存审核事件；`checkpoint-manifest.json` 记录 checkpointer 类型、存储方式、连接环境变量名和 checkpoint 摘要。目标态使用 `postgres` 模式，checkpoint 数据保存到 PostgreSQL，不再依赖 pickle 文件作为恢复主路径。

当前 Runtime 使用运行级持久化 checkpointer：

- 每个 run 使用稳定的 `thread_id`。
- `StateGraph` 执行期间每次 checkpoint 写入都会保存到 PostgreSQL checkpointer。
- `runtime.checkpointer=postgres`，Runtime 使用 `langgraph-checkpoint-postgres` 的 `PostgresSaver`，连接串只从 `runtime.checkpoint_postgres_dsn_env` 指定的环境变量读取。
