...
<!-- 注意：runtime 同时加载 docs/api-test-generation.md 与本文件（prompts/api-test-generation-prompt.md），需保持一致；本文件为 docs/ 运行时辅助版本，规范以 prompts/ 版本为权威 -->

# 接口测试草稿生成 Prompt（docs 运行时辅助版）

> 权威契约来源：`AGENTS.md`、`runtime/workspace.py`（`artifacts/` 路径）、`runtime/llm/prompt_builder.py`、`runtime/graph/nodes/mvp_quality.py`。本文件与 `prompts/api-test-generation-prompt.md` 同时被 Runtime 加载，规范必须保持一致：路径统一 `prd/<id>/artifacts/`，脚本落 `prd/<id>/automation/api/`，禁止 `analysis/`、`cases/`、`defects/`、`execution/`、`report/` 子目录。
...
根据接口文档、需求分析与已审核测试用例，生成 API 测试计划、测试数据设计、环境变量说明、pytest 脚本草稿、断言策略与可机读 YAML 用例。

## 任务目标

输出 `prd/<id>/artifacts/api-test-draft.md` 草稿（含可执行 pytest 脚本），停在 `needs_human_review`；默认不得连接生产环境，不代替人工审核，不输出正式执行结论。Runtime 会为同一 run 生成 `prd/<id>/api-test-cases.yml` 等可机读产物，必须与本草稿使用相同的接口事实来源与待确认项。

## 输入

- 原始需求：`prd/<id>/input/requirement.md`
- 接口文档（优先）：`prd/<id>/input/api.md`
- OpenAPI / Swagger / Apifox 导出（可选，归一化为 `input/api.md` 后优先使用）：`prd/<id>/input/api.openapi.json|yaml`、`prd/<id>/input/api.swagger.json|yaml`、`prd/<id>/input/api.apifox.json|yaml`
...
### 可机读 YAML 用例（Runtime 辅助产物）

Runtime 会基于本草稿生成 `prd/<id>/api-test-cases.yml`，YAML 用例必须遵守：

- 只能写相对 path，不得写完整域名；base URL 统一从 `AGENTIC_QA_BASE_URL` 读取。
- token、Cookie、账号等只能使用 `${ENV_NAME}` 占位，不得写真实值。
- 必须包含 `business_rules` 与每条用例的 `business_rule_refs`，用于追溯 PRD / Swagger / 业务规则来源。
- 用例只能使用同一接口事实来源（归一化后的 `input/api.md`），待确认项与本草稿一致。
...
6. 接口事实（URL/Method/字段）来自 `input/api.md`（含归一化的 OpenAPI/Swagger/Apifox）；未确认项必须写成待人工审核点，不得臆造。
7. 不执行真实 HTTP 请求、不输出执行结论、不写真实 token/Cookie/密钥。
8. 有 OpenAPI / Swagger / Apifox 导出时，URL、Method、请求字段、响应字段必须来自归一化接口文档。
9. 无可用 `input/api.md` 时，只能输出接口候选点，必须标记「待补充接口文档」。
...
1. **理解契约**：通读 `input/api.md` 与 `artifacts/requirement-analysis.md`，确认路径、方法、请求/响应结构；若有 OpenAPI/Swagger/Apifox，先归一化为 `api.md`。
...
5. **检查约束**：是否硬编码敏感信息？是否未设环境变量即请求真实服务？YAML 用例是否用 `${ENV}` 占位且只含相对 path？是否所有未确认项已写入待审核点？
</instructions>
...
| YAML 合规 | 相对 path、token/Cookie 用 `${ENV}`、含 `business_rules` 与 `business_rule_refs` |
...
- 不把未确认假设当接口事实。
- 不输出「待接入后生成」或仅少量示例脚本。
- YAML 用例不得写完整域名或真实 token/Cookie/账号。
...
- 测试数据是否可用
- 环境变量名与运行环境是否匹配
- OpenAPI/Swagger/Apifox 导出是否需要补充
...
| OpenAPI/Swagger/Apifox | 产品/开发 | `prd/<id>/input/api.*.json|yaml` | 先归一化为 `api.md` |
...
### Q: YAML 用例的路径和凭据怎么写？
只写相对 path（不含域名），base URL 走 `AGENTIC_QA_BASE_URL`；token/Cookie/账号一律 `${ENV_NAME}` 占位，绝对不写真实值。
...
6. YAML 用例（如有）仅含相对 path、使用 `${ENV}` 占位、含 `business_rules` 与 `business_rule_refs`。
...
- 仅 OpenAPI 导出无 `api.md` → 先归一化为 `api.md` 再生成，字段来源可追溯。

## 版本记录

| 版本 | 日期 | 变更说明 |
|------|------|----------|
| v2.1 | 2026-07-13 | 与 prompts/api-test-generation-prompt.md 对齐：补 14 章结构与 FM；路径 `prd/<id>/artifacts/api-test-draft.md`、脚本 `prd/<id>/automation/api/`；保留 OpenAPI/Swagger/Apifox 归一化、YAML schema（`AGENTIC_QA_BASE_URL`/`${ENV}`/`business_rules`）约束；新增「成功标准与验证」；顶部一致注释 |
