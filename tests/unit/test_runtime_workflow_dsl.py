...
def test_load_runtime_workflow_spec_from_yaml():
    ...
    assert spec.state["task_type"] == "analysis_and_testcases"
    ...
    assert any(edge.condition == "task_is_analysis_and_testcases" for edge in spec.edges)
...
def test_graph_app_facades_use_runtime_workflow_specs(tmp_path):
    ...
    testcases = graph_app.run_testcase_generation_workflow(
        "generate testcases",
        "prd/demo-requirement",
        repo_root=repo_root,
        record_run=False,
    ...
    combined = graph_app.run_analysis_and_testcases_workflow(
    ...
    assert combined.orchestration == "YAML WorkflowSpec: analysis_and_testcases"
    ...
    assert combined.task_type == "analysis_and_testcases"
...
def test_resume_recorded_workflow_routes_combined_task_type_to_runtime_dsl(tmp_path):
    repo_root = create_runtime_repo(tmp_path)
    result = run_workflow_by_id(
        "testcase_generation",
        "generate testcases",
