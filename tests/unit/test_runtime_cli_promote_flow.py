...
import pytest
import yaml
from runtime_fixtures import create_runtime_repo, write_promote_fixture
...
def test_cli_promote_command_without_artifact_publishes_latest_combined_run_keys(tmp_path):
    repo_root = create_runtime_repo(tmp_path)
...
def test_cli_resume_command_retry_failed_run(tmp_path):
    ...
    assert exit_code == 0
    assert (
        repo_root / f"prd/demo-requirement/runs/{result.run_id}/testcases.preview.md"
    ).is_file()
    assert not (repo_root / "prd/demo-requirement/artifacts/testcases.md").exists()
