...
```bash
python -m runtime.cli "分析 prd/sample-login-requirement 并生成测试用例"
```

自然语言确认并发布正式产物：

```bash
python -m runtime.cli "测试用例通过，发布正式产物 prd/sample-login-requirement"
```
...
```bash
python -m runtime.cli promote prd/sample-login-requirement run-20260616-060850-0ec07a testcases
```
...
- 目标工作区：`prd/sample-login-requirement`
- 目标产物：需求分析候选内容
...
```text
测试用例通过，发布正式产物 prd/sample-login-requirement。
```

```text
需求分析和测试用例都通过，发布正式产物 prd/sample-login-requirement。
```
...
```bash
python -m runtime.cli promote prd/sample-login-requirement testcases
python -m runtime.cli promote prd/sample-login-requirement run-20260616-060850-0ec07a testcases
python -m runtime.cli promote prd/sample-login-requirement run-20260616-060850-0ec07a requirement_analysis
```

说明：

- 第一条未显式指定 `run_id`，Runtime 会读取目标工作区的 latest run 指针，并只发布测试用例。
...
```text
为 prd/sample-login-requirement 生成测试用例，重点覆盖边界值和异常流程。
```
...
```text
run-20260616-060850-0ec07a 的测试用例通过，发布正式产物 prd/sample-login-requirement。
```
