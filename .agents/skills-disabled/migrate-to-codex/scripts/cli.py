...
def convert_tree(
    ...
    for scope_name, scope in zip(SCOPE_NAMES, scopes, strict=False):
...
def render_migration_report(
    ...
    if deploy_mode == DeployMode.REPLACE:
        ...
        for orphan in deployment_plan.orphaned_skill_dirs:
            lines.append(f"  {orphan_status}: {orphan.as_posix()} - Orphaned generated skill.")
        for orphan in deployment_plan.orphaned_agent_files:
            lines.append(f"  {orphan_status}: {orphan.as_posix()} - Orphaned generated subagent.")
    return "\n".join(lines)
...
def deploy_tree(
    ...
    for scope_name in SCOPE_NAMES:
        ...
        scope_artifacts = tuple(artifact.without_prefix() for artifact in prefixed_scope_artifacts)
...
def selected_components(args: argparse.Namespace) -> frozenset[str]:
    components = {
        component for component in ("mcp", "skills", "subagents") if getattr(args, component, False)
    }
    if not components:
...
def render_migration_plan(
    ...
    manual_items = [
        item for item in conversion_result.report_items if item.status == "manual_fix_required"
    ]
    if manual_items:
...
def render_doctor_report(
    ...
    lines.append(
        "  recommended flow: run --plan, run --dry-run, review manual items, migrate, then run --validate-target."
    )
    return "\n".join(lines)
