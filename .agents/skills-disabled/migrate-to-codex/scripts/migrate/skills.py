...
COMMAND_FILE_SOURCES = ((Path(".claude") / "commands", "source-command", "source command"),)

SKILL_SOURCE_ROOTS = (Path(".claude") / "skills",)
...
def validate_skill_files(target_root: Path) -> list[MigrationReportItem]:
    ...
    for skill_file in sorted(skills_root.glob("*/SKILL.md")):
        ...
        missing = [
            key for key in ("name", "description") if not document.frontmatter.optional_string(key)
        ]
        if missing:
            report_items.append(
                MigrationReportItem(
