...
def parse_yaml_value(value: str) -> YamlValue:
    ...
    if value.startswith("[") and value.endswith("]"):
        return tuple(parse_yaml_value(item) for item in split_delimited_values(value[1:-1]) if item)
    if value.startswith('"') and value.endswith('"'):
        try:
...
def format_yaml_mapping(values: Mapping[str, YamlValue]) -> str:
    return "\n".join(f"{key}: {format_yaml_value(value)}" for key, value in values.items())
