...
- URL: `https://figma.com/design/figma-file-key/DesignSystem?node-id=42-15`
...
1. Parse URL to extract fileKey=`figma-file-key` and nodeId=`42-15`
2. Run `get_design_context(fileKey="figma-file-key", nodeId="42-15")`
3. Run `get_screenshot(fileKey="figma-file-key", nodeId="42-15")` for visual reference
...
1. Parse URL to extract fileKey=`figma-file-key` and nodeId=`10-5`
2. Run `get_metadata(fileKey="figma-file-key", nodeId="10-5")` to understand the page structure
3. Identify main sections from metadata (header, sidebar, content area, cards) and their child node IDs
4. Run `get_design_context(fileKey="figma-file-key", nodeId=":childNodeId")` for each major section
5. Run `get_screenshot(fileKey="figma-file-key", nodeId="10-5")` for the full page
