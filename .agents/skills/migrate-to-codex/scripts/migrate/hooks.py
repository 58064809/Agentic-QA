    ...
    def from_scope(cls, scope_root: Path) -> ClaudeHooks:
        ...
        return cls(
            matcher_groups=tuple(
                matcher_group for hook_set in hook_sets for matcher_group in hook_set.matcher_groups
            ),
            source_paths=tuple(
                source_path for hook_set in hook_sets for source_path in hook_set.source_paths
            ),
            ...
            for group_config in json_object_tuple(groups_value):
                ...
                for hook_config in json_object_tuple(group_config.get("hooks")):
                    ...
                    if hook_type != "command":
                        unsupported_fields.append(f"hooks.{event_name}.hooks[].type:{hook_type}")
                        continue
